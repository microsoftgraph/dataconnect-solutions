...
The RegisteredDevices_v0 dataset can be joined with relevant users, and other category datasets.

## Definitions

- A user may register some personal devices as well as be registered on some company devices. Devices registered by the user can have certain configurations and specifications that are stored. The configs can be set up in any one of the management channels opted for the particular device. The details of the onPrem devices are also stored and can be retreived by the tenants. 
