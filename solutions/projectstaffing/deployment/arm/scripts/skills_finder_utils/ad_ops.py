...
def add_gdc_deployer_identity(resource_group: str, gdc_admin_ad_group: str, identity_name: str = "gdc-deployer"):
    ...
    if len(id_principals) == 0:
        ...
        if not rsp:
            print("Warning: couldn't find gdc-deployer identity ")
    ...
    _add_member_to_group(existing_group_name=gdc_admin_ad_group, user_or_identity_obj_id=identity['principalId'])
...
@retry(tries=5, delay=1, backoff=2)
def _get_access_token(resource_id: str = None):
    rsp = dict()
    if resource_id:
        rsp = az_cli(" account get-access-token ", "--resource", resource_id)
    else:
        rsp = az_cli(" account get-access-token ")
...
def get_databricks_access_token():
    ...
    return _get_access_token(resource_id=resource_id)
