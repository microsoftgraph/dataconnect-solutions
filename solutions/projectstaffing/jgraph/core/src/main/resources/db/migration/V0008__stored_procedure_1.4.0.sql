create or alter procedure [dbo].[find_recommended_employees]
    ...
    @latest_hr_data_version_str                     VARCHAR(32),
...
begin
    ...
    declare
        ...
        @sql_hr_data_join_type 						VARCHAR(MAX),       -- type of join query with HR Data. Depends on @is_hr_data_mandatory
        ...
        @latest_hr_data_version 					DATETIME2,
    ...
    set @latest_hr_data_version = CONVERT(datetime2, ISNULL(@latest_hr_data_version_str, '1753-01-01 00:00:00.000000'));
    ...
    if @order_by_availability_and_name = 1
        set @sql_order = '(CASE WHEN CONVERT (date, hr.available_starting_from, 23) IS NULL THEN 0 WHEN hr.available_starting_from = '''' THEN 0 ELSE 1 END), (CASE WHEN hr.available_starting_from = '''' THEN NULL ELSE CONVERT (date, hr.available_starting_from, 23) END) ASC, name ASC ';
    ELSE
    	...
    	begin
    	    ...
    	    begin
    		    ...
    		    set @table_name = case @table_name
    		                      ...
    		                      WHEN 'hr_data_employee_profile' THEN 'hr'
    		                      end
    ...
    IF @available_since IS NOT NULL AND @available_since <> ''
        begin
            set @sql_where = @sql_where + ' AND ( hr.available_starting_from IS NULL OR hr.available_starting_from = '''' OR CONVERT (date, hr.available_starting_from, 23) <= CONVERT (date, ''' + @available_since + ''' , 23) ) ';
        end
    ...
    IF @is_m365_first_data_source = 1
        begin
            set @location_query = 'COALESCE(NULLIF(CONCAT_WS('', '', NULLIF(ep.city, ''''), NULLIF(ep.state, ''''), NULLIF(ep.country, '''')), ''''), hr.location)'
            set @role_query = 'COALESCE(NULLIF(ep.job_title, ''''), hr.[role])'
            set @reports_to_query = 'COALESCE(NULLIF(ep.reports_to, ''''), hr.manager_name)'
            set @manager_email_query = 'COALESCE(NULLIF(ep.manager_email, ''''), hr.manager_email)'
        end
    ELSE
        begin
            set @location_query = 'COALESCE(NULLIF(hr.location, ''''), CONCAT_WS('', '', NULLIF(ep.city, ''''), NULLIF(ep.state, ''''), NULLIF(ep.country, '''')))'
            set @role_query = 'COALESCE(NULLIF(hr.[role], ''''), ep.job_title)'
            set @reports_to_query = 'COALESCE(NULLIF(hr.manager_name, ''''), ep.reports_to)'
            set @manager_email_query = 'COALESCE(NULLIF(hr.manager_email, ''''), ep.manager_email)'
        end
    ...
    SET @sql = @sql + '
			SELECT
				...
				hr.current_engagement  		as current_engagement,
				ep.department                   as department,
				' + @reports_to_query + '       as reports_to,
				' + @manager_email_query + '    as manager_email,
				CASE WHEN hr.available_starting_from = '''' THEN NULL ELSE CONVERT (date, hr.available_starting_from, 23) END		-- https://www.mssqltips.com/sqlservertip/1145/date-and-time-conversions-using-sql-server/
												as available_since,
				hr.linkedin_profile  		    as linkedin_profile,
			...
			' + @sql_hr_data_join_type + ' JOIN
			   (select * from hr_data_employee_profile where version = ''' + CONVERT(varchar, @latest_hr_data_version) +  ''') hr
			ON ep.mail = hr.mail
