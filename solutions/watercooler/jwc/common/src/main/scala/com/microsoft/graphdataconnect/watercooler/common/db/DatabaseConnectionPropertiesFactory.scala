...
import com.microsoft.graphdataconnect.watercooler.common.security.ADLoginService
import com.microsoft.graphdataconnect.watercooler.common.secret.Vault
import com.microsoft.graphdataconnect.watercooler.common.security.{ADLAccessToken, ADLoginService}
  ...
  def getProperties(applicationId: String,
    ...
    if (useMsiAzureSqlAuth) {
      val adlAccessToken: ADLAccessToken = ADLoginService(directoryId).loginForAccessToken(clientId = applicationId,
        ...
        applicationSecretKeyName = adbSPClientKeySecretName,
        scope = scope)

      connectionProperties.put("accessToken", adlAccessToken.accessToken)
    } else {
      if (jdbcUsernameKeyName.nonEmpty && jdbcPasswordKeyName.nonEmpty) {
