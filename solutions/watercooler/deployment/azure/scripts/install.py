...
def init_active_directory_entities(deployment_name: str, install_config: InstallConfiguration, resource_group: str):
    ...
    if not install_config.wc_admin_ad_group:
        print(
            "The deployment admins group defines list of AD users which are going to have Owner role over all Azure resources created by this deployment.")
        print(
        ...
        admin_ad_group = ad_ops.prompt_or_create_ad_group(
            "Enter the name or id of an existing Active Directory group for deployment admins: ",
    ...
    if not install_config.wc_service_principal:
        print("Creating %s service principal " % "wc-service")
        ...
        wc_sp = ad_ops.get_or_create_service_principal("wc-service")
    ...
    if not install_config.m365_reader_service_principal:
        print("Creating %s service principal " % "wc-m365-reader")
        graph_user_read_all_role = ad_ops.find_graph_user_read_all_role()
        if not graph_user_read_all_role:
            raise RuntimeError("Couldn't find 'User.Read.All' permission in 'Microsoft Graph' for your tenant ")

        m365_reader_sp = ad_ops.get_or_create_service_principal("wc-m365-reader")
        ...
        try:
            ...
            print("Failed to make members of admin group owners over wc-m365-reader service principal!")
            print(azError)
    ...
    if not install_config.watercooler_user:
        print("Creating Watercooler Meetings Organizer user")
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--log-analytic-enabled', default=False, required=False,
    ...
    if install_state.is_user_prompts_taken():
        if install_state.prompt_stage_repeat(
                "Previously entered values have been found. Would you like to ignore them and re-enter deployment parameters? (Y/n) "):
            execute_user_prompts(deployment_name=deployment_name, install_config=install_config,
